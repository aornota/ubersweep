...
type UserMapper(userLists: (Guid * Events.User) list list) =
    let userDic = Dictionary<string, Guid * Events.User>()

    do
        userLists
        |> List.iter (fun users ->
            users
            |> List.iter (fun (guid, user) ->
                if userDic.ContainsKey user.UserName then
                    userDic[user.UserName] <- guid, user
                else
                    userDic.Add(user.UserName, (guid, user))))

    member _.Users() = userDic.Values |> List.ofSeq

    member _.MapUserId(sourceUserDic: Dictionary<Domain.UserId, string>) : MapUserId =
        fun userId ->
            if sourceUserDic.ContainsKey userId then
                let userName = sourceUserDic[userId]

                if userDic.ContainsKey userName then
                    UserId.FromGuid(fst userDic[userName])
                else
...
type Migration(config: IConfiguration, persistenceFactory: IPersistenceFactory, logger) =
    ...
    member this.MigrateAsync() = asyncResult {
        ...
        // TODO-MIGRATION: Improve / abstract this...
        ...
        (* TODO:
            ...
            -- for each "partition":
                ...
                - create User mapping...
                - write migrated Users (as snapshots?)...
                - write migrated events (or snapshots?)...*)
